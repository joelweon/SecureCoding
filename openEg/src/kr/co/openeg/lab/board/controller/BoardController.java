...
public class BoardController {
	...
	public void getImage(HttpServletRequest request, HttpSession session, 
		...
		// 읽어올 파일 경로 (저장된 파일경로)
		String filePath = session.getServletContext().getRealPath("/")+"WEB-INF/files/" + board.getFileName();
		...
		try {
			...
			response.setHeader("Content-Disposition",
					           "inline;filename=" + board.getFileName()); //inline 렌더링후 뿌리기 attach저장화면
			// 파일로 부터 입력 받도록 설정
	...
	public String boardWriteProc(@ModelAttribute("BoardModel") BoardModel boardModel, MultipartHttpServletRequest request, HttpSession session){
		...
		// 파일의 타입->이미지  이미지 확장자 -> .jpg
		//  파일의 크기가 10M 보다 작은 경우 업로드 허용
		if ( file != null
